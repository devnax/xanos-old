...
import { isArray, isNull, isInteger, isObject, isString, isNumber, isBool, isFunction, isUrl, isEmail } from './utils'
...
export const object = define('object', ({ value, args, field }) => {
   ...
   for (let field in args[0]) {
      ...
      if (!value.hasOwnProperty(field) && factory.name !== 'optional') {
...
export const number = define('number', ({ value, field }) => {
   const err = watch(enums(string(), integer()), value)
   if (err) {
      throw new Error(err.message);
   }
   if (!isNumber(value)) {
...
export const enums = define('enums', ({ value, args, field }) => {
   ...
   args.forEach((v: any) => {
      if (v instanceof SchemaFactory && watch(v, v)) {
         has = true
      } else if (value === v) {
         has = true
      }
...
export const email = define('email', ({ value, field }) => {
   try {
      blast(string().field(field), value)
      if (!isEmail(value)) {
